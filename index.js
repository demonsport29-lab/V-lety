...
app.get('/tajny-import-akci', async (req, res) => {
    try {
        const noveAkce = [
            {
        ...
        res.json({ uspech: true, zprava: "Všechny akce uloženy!", detail: `Přidáno: ${pridano}, Přeskočeno (duplicity): ${preskoceno}` });
    } catch (err) {
        console.error('Chyba při tajném importu:', err);
        res.status(500).json({ uspech: false, chyba: err.message });
    }
});
