    ...
    v.forEach((x,i)=>{
        ...
        k.innerHTML=`
            ...
            <div class="ar-unified" style="display:grid; grid-template-columns: repeat(2, 1fr); gap:8px; margin-top:16px;">
                ...
                <button class="btn bg bi btn-qr" onclick="event.stopPropagation();generovatQRVyletu('${x.shareId}')" style="justify-content:center; border-radius:10px;"><i class="ti ti-qrcode"></i> QR Kód</button>
...
async function generovat(){
    ...
    const btn=document.getElementById('genBtn');btn.innerHTML='<div class="spin"></div> Zpracovávámâ€¦';btn.disabled=true;
    try{
        ...
        if(res.uspech){
            ...
            if (curDraft.pocasi && curDraft.pocasi.teplota !== undefined) {
                const wmoKody = {
                    ...
                    80:{i:'ti-cloud-storm wa-pulse wa-bolt', t:'Přeháňky'}, 
                    81:{i:'ti-cloud-storm wa-pulse wa-bolt', t:'Silné přeháňky'}, 
                ...
                wBox.innerHTML = `
                    <div style="display:flex; align-items:center; gap:16px; background:rgba(255,255,255,0.06); padding:12px 18px; border-radius:18px; border:1px solid rgba(255,255,255,0.1); width:max-content; margin-top:14px; box-shadow:0 8px 32px rgba(0,0,0,0.15);">
                        ...
                        <div>
                            ...
                            <div style="font-size:0.75rem; color:var(--t2); font-weight:600;">${wip.t} &nbsp;â€˘&nbsp; Vítr ${curDraft.pocasi.vitr} km/h</div>
                        </div>
                    </div>`;
            ...
            // BEZPEČNĂ‰ SKRYTĂŤ TLAČĂŤTKA SDĂŤLENĂŤ
...
function ukazAchievementModal(ach) {
    ...
    // Spustit konfety (pokud máme libku, nebo aspoň visual feedback)
...
// NATIVNĂŤ SDĂŤLENĂŤ (Nyní odkazuje na konkrétní výlet)
...
async function doKalendare() {
    ...
    if (typeof curDraft !== 'undefined' && curDraft?.etapy) {
        curDraft.etapy.forEach(e => {
            detailniPopis += `đź•’ ${e.cas} — ${e.misto}\nđź“ť ${e.popis}\n\n`;
        });
    }
...
function otevritDetailVyletu(v){
    ...
    // BEZPEČNĂ‰ ZOBRAZENĂŤ TLAČĂŤTKA SDĂŤLENĂŤ
    const shareBtn = document.getElementById('btnShareTrip');
        ...
        wBox.innerHTML = `
            <div style="display:flex; align-items:center; gap:16px; background:rgba(255,255,255,0.06); padding:12px 18px; border-radius:18px; border:1px solid rgba(255,255,255,0.1); width:max-content; margin-top:14px; box-shadow:0 8px 32px rgba(0,0,0,0.15);">
                ...
                <div>
                    ...
                    <div style="font-size:0.75rem; color:var(--t2); font-weight:600;">${wip.t} &nbsp;â€˘&nbsp; Vítr ${v.pocasi.vitr} km/h</div>
                </div>
            </div>`;
        wBox.style.display = 'block';
    } else { wBox.style.display = 'none'; }
...
// 3. UPRAVENĂť FEED - Dělá jména a avatary klikací
...
// 4. ZCELA NOVĂ‰ FUNKCE PRO VEŘEJNĂ‰ PROFILY
                ...
                <div style="display:flex; flex-direction:column; gap:30px;">
                    <div style="display:flex; align-items:center; gap:20px; background:rgba(255,255,255,0.03); padding:24px; border-radius:24px; border:1px solid rgba(255,255,255,0.05);">
                        ...
                        <div style="flex:1;">
                            ...
                            <p style="color:var(--t2); font-size:0.9rem;">Členem od: ${new Date(u.datumRegistrace).toLocaleDateString()}</p>
                        </div>
                    ...
                    <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap:12px;">
                        ...
                        <div class="dc au" style="text-align:center; padding:16px; border-radius:20px;">
                            ...
                            <span style="font-size:0.65rem; color:var(--t2); text-transform:uppercase; font-weight:800; letter-spacing:0.05em;">Ăšspěchy</span>
                        </div>
                    </div>
...
// VYKRESLENĂŤ AKCĂŤ (S perfektně vycentrovanými SVG ikonami)
...
async function toggleEditTrip() {
    ...
    if (!isEditingTrip) {
        ...
        btn.innerHTML = "đź’ľ Uložit úpravy";
        btn.style.width = "auto";
        btn.style.padding = "0 14px";
        btn.classList.replace("bgh", "bp"); // Zvýraznění tlačítka
        
    } else {
        // VYPNUTĂŤ A ULOŽENĂŤ ĂšPRAV
        isEditingTrip = false;
