...
async function generovat(){
    ...
    try{
        ...
        if(res.uspech){
            ...
            nactiPocasiOpenMeteo(curDraft.lokace);
...
window.nactiPocasiOpenMeteo = async function(nazevLokace) {
    ...
    try {
        ...
        console.error('API Počasí selhalo a nezdařilo se načíst data přímo od klienta:', err);
    }
}
...
function vykreslitDopravu(draft) {
    ...
    if (draft && draft.doprava && draft.doprava.typ === 'vlak') {
        ...
        d.innerHTML = 
            ...
            <p style="font-size:0.9rem; color:var(--t2); margin-bottom:12px;">Nejpohodln�j�� cesta je vlakem z \ do \.</p>
            <a href="https://www.cd.cz/spojeni-a-jizdenka/spojeni-tam?\" target="_blank" class="btn" style="background:#0055A5; color:white; border:none; padding:12px 20px; border-radius:10px; font-weight:700; display:inline-flex; align-items:center; gap:8px;">
